...
#ifdef DARKINTERVAL // classic fire from 2004
#define CLASSIC_FIRE
#endif
...
#ifdef DARKINTERVAL // loop and ignition sounds built into the fire to save on entity counts
DEFINE_SOUNDPATCH(m_pLoopSound),
DEFINE_KEYFIELD(m_LoopSoundName, FIELD_SOUNDNAME, "LoopSound"),
DEFINE_KEYFIELD(m_StartSoundName, FIELD_SOUNDNAME, "IgniteSound"),
#endif
...
CFire::CFire(void)
{
	...
	m_StartSoundName = MAKE_STRING("coast.gaspump_ignite");
#endif
...
{
	if (inputdata.value.Float() == NULL)
	{
		Msg("input to fire system %s ExtinguishTemporary missing float parameter, using default value of 1.0 seconds\n", GetDebugName());
		GoOutInSeconds(1.0f);
	}
	else
		GoOutInSeconds(inputdata.value.Float());
...
void CFire::Spawn(void)
{
	...
	//	m_spawnflags &= ~SF_FIRE_SMOKELESS;
...
void CFire::Update(float simTime)
{
	...
	{
		m_flDamageTime = gpGlobals->curtime + 1.0f;
		outputDamage = (1.0f + outputHeat * 0.1f * m_flDamageScale);
		if (outputDamage)
		{
			damage = true;
		}
...
void CFire::AddHeat(float heat, bool selfHeat)
{
	if (m_bEnabled)
	{
		...
		if (m_flHeatAbsorb > 0)
		{
			float absorbDamage = heat * 3.0f;
			if (absorbDamage > m_flHeatAbsorb)
			{
				heat -= m_flHeatAbsorb / 3.0f;
				m_flHeatAbsorb = 0;
			}
			else
			{
...
void CFire::Extinguish(float heat)
{
	...
	m_flHeatLevel -= heat;
	m_flHeatAbsorb += 5.0f * heat;
	if (m_flHeatAbsorb > 50.0f)
	{
		m_flHeatAbsorb = 50.0f;
	}
...
#define SF_FIRESENSOR_SENSE_FIRESOURCES	2
#define SF_FIRESENSOR_SENSE_FLARES	4
...
void CEnvFireSensor::Think()
{
	...
	// DI NEW
	if (HasSpawnFlags(SF_FIRESENSOR_SENSE_FIRESOURCES))
	{
		CBaseEntity* pTouch = NULL;
		while ((pTouch = gEntList.FindEntityInSphere(pTouch, GetAbsOrigin(), m_radius)) != NULL)
		{
			CEnvFireSource* pSource = dynamic_cast<CEnvFireSource*>(pTouch);
			if (pSource && pSource->IsFireSourceActive())
			{
				heat += pSource->GetHeatLevel();
			}
		}
	}
	//
