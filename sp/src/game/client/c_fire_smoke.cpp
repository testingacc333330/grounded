// This code is taken and fixed up from Dark Interval, dedacted was here though :P
// https://github.com/Cvoxalury/darkinterval-opensource/blob/main/src/game/client/c_fire_smoke.cpp
...
void C_FireSmoke::Start(void)
{
	...
	if (m_nFlags & bitsFIRESMOKE_VISIBLE_FROM_ABOVE)
	{
		for (int di = 0; di < NUM_CHILD_FLAMES; di++)
		{
			if (bTools)
			{
				ClientEntityList().AddNonNetworkableEntity(&m_entFlamesFromAbove[di]);
			}
			m_entFlamesFromAbove[di].AddToLeafSystem(RENDER_GROUP_TRANSLUCENT_ENTITY);
		}
	}
	...
	if (m_nFlags & bitsFIRESMOKE_VISIBLE_FROM_ABOVE)
	{
		for (int di = 0; di < NUM_CHILD_FLAMES; di++)
		{
			...
			//Setup all the information for the client entity
			m_entFlamesFromAbove[di].SetModelByIndex(m_nFlameFromAboveModelIndex);
			m_entFlamesFromAbove[di].SetLocalOrigin(GetLocalOrigin());
			m_entFlamesFromAbove[di].m_flFrame = Helper_RandomInt(0.0f, maxFrames - 1);
			m_entFlamesFromAbove[di].m_flSpriteFramerate = Helper_RandomInt(15, 30);
			m_entFlamesFromAbove[di].SetScale(m_flStartScale);
			m_entFlamesFromAbove[di].SetRenderMode(kRenderTransAddFrameBlend);
			m_entFlamesFromAbove[di].m_nRenderFX = kRenderFxNone;
			m_entFlamesFromAbove[di].SetRenderColor(255, 255, 255, 255);
			m_entFlamesFromAbove[di].SetBrightness(255);
			m_entFlamesFromAbove[di].AddEffects(EF_NORECEIVESHADOW | EF_NOSHADOW);

			m_entFlamesFromAbove[i].m_nIndex = -1;

			if (di == 0)
			{
				m_entFlameScales[di] = 1.0f;
			}
			else
			{
				//Keep a scale offset
				m_entFlameScales[di] = random->RandomFloat(0.5f, 1.0f);
			}
		}
	}
...
void C_EntityFlame::Simulate(void)
{
	...
	{
		dlight_t* dl = effects->CL_AllocDlight(GetInde);
