...
void CRagdollLRURetirement::MoveToTopOfLRU(CBaseAnimating* pRagdoll, bool bImportant)
{
	if (bImportant)
	{
		...
		if (m_LRUImportantRagdolls.Count() > g_ragdoll_important_maxcount.GetInt())
		{
			...
			pRagdoll = m_LRUImportantRagdolls[iIndex].Get();
...
#ifdef DARKINTERVAL
#ifdef CLASSIC_FIRE
		// DI FIX: this is how to fix infinite fire spawning in the origin point of the ragdoll. (remember CW3D stream)
...
#if defined (CLASSIC_FIRE) && defined( DARKINTERVAL )
{// DI FIX: this is how to fix infinite fire spawning in the origin point of the ragdoll. (remember CW3D stream)
